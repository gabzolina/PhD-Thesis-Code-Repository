...
def prepMoisture01():
    "measures the moisture of the material through first sensor"
    # Create the I2C bus
    ...
    chan01 = AnalogIn(ads, ADS.P0)
    

    print("{:>5}\t{:>5}\t{:>5}".format('raw01','p01', 'v01'))
   
   
    if True:
        m01percentage = (chan01.value * 100) / 20432
        print("{:>5}\t{:>5.0f}\t{:>5.5f}".format(chan01.value, m01percentage, chan01.voltage))
        
    return m01percentage
    
     
        
def prepMoisture02():
    "measures the moisture of the material through second sensor"
    # Create the I2C bus
    i2c = busio.I2C(board.SCL, board.SDA)
    ...
    chan02 = AnalogIn(ads, ADS.P1)
    

    print("{:>5}\t{:>5}\t{:>5}".format('raw02','p02', 'v02'))
   
   
    if True:
        m02percentage = (chan02.value * 100) / 20432
        print("{:>5}\t{:>5.0f}\t{:>5.5f}".format(chan02.value, m02percentage, chan02.voltage))
        
    return m02percentage
...
def CombineImages(PiImage, LeptonImage):
    ...
    pts_src = np.array([[2, 51], [155, 50], [3, 86],[155, 85]])

    # Four corners of the sample in destination image (pi camera output)
    #- these are manually found in photoshop and placed here
    pts_dst = np.array([[257, 615],[1859, 605],[243, 1003],[1879, 969]])
    ...
    #cv2.imwrite('finaloutputSB1.png',outputFinal)
